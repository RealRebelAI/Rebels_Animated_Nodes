...
const ANIMATIONS = {
    "Matrix Rain": {
        ...
        draw: (ctx, w, h, t, n, colors) => {
            // EXACT Fractional Stepper from the original Monitor
...
app.registerExtension({
    ...
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if (nodeData.name === "Matrix_Monitor") {
            ...
            // Capture native background draw if it exists
            ...
            nodeType.prototype.onDrawBackground = function(ctx) {
                ...
                // Draw any native extensions over the theme
    ...
    async setup() {
        api.addEventListener("rebels_matrix_log", (event) => {
            const logMessage = event.detail.message;
            const monitorNodes = app.graph.findNodesByType("Matrix_Monitor");
            
            monitorNodes.forEach(node => {
                if (node.logContainer && node.widgets[0].value === "Active") {
                    ...
                    } else if (logMessage.includes("got prompt")) {
...
app.registerExtension({
    ...
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        ...
        if (nodeData.name.startsWith("RebelAnimated")) {
            ...
            // THE FIX: Save the native onDrawBackground hook before overwriting it
            const origOnDrawBackground = nodeType.prototype.onDrawBackground;
            ...
            nodeType.prototype.onDrawBackground = function(ctx) {
                ...
                ctx.restore(); // Important to restore before calling native functions
                
                // THE FIX: Call ComfyUI's native drawing function AFTER our animation.
                // This ensures image previews (like LoadImage and SaveImage) draw ON TOP of the theme!
                if (origOnDrawBackground) {
                    origOnDrawBackground.apply(this, arguments);
                }
