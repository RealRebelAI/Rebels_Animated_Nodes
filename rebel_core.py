...
class ComfyLogInterceptor:
    def __init__(self):
        ...
        sys.stdout = self
        # Safely expanded tags. (We removed stderr to prevent boot crashes).
        self.target_tags = [
            "[StagedJE]", "[Rebels JE]", "[JoyEcho]", "[INFO]", 
            "Prompt executed", "ERROR", "got prompt", 
            "Loading", "loading", "Loaded", "loaded", 
            "fetching", "Model", "VAE", "CLIP", "Decoding", "Saved", "Requested"
        ]
    ...
    def write(self, buf):
        ...
        try:
            clean_buf = buf.strip()
            if clean_buf and any(tag in clean_buf for tag in self.target_tags):
                PromptServer.instance.send_sync("rebels_matrix_log", {"message": clean_buf})
        except Exception:
            pass
...
# --- STRICT DIFFUSION LOADER (diffusion_models folder) ---
...
# --- STRICT BASE UNET LOADER (unet folder) ---
...
# --- STRICT GGUF UNET LOADER (unet folder) ---
